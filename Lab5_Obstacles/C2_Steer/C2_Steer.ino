...
int  InIndex=0;
int  LowInNibble = true;
int  throttle_control = MinimumThrottle;
int  brake_control = FullBrake;
int  steer_control = Straight;
long speed_errors[ERROR_HISTORY];
// SteerAngle is in degrees x 10. 
// 0 is straight; 900 right, -900 left.
// It is relative to the trike, and independent of which way the trike points.
int  SteerAngle_degX10 = 0;
int  StraightReading = 0;
...
void initialize()
{
  ...
  for (tries = 0; StraightReading != NaN; tries++)
  {
    delay(100);
    StraightReading = getSteerAngle();
    if (tries > 10)
    {
      Serial.println("Steering encoder failed.");
      break;
    }
  }
}
...
void loop()
{
	...
	if ( Serial3.available() > 0) 
	{
	    ...
	    if (LowInNibble)
	    {
		IncomingMessage[InIndex] = (char)(0x0F & incomingByte);	
		LowInNibble = false;
	    }
	    else
	    {
		LowInNibble = true;
		IncomingMessage[InIndex] = (char)(0x0F0 & (incomingByte<<4));
 ...
 // apply steering
  if (StraightReading == NaN)
  {
     StraightReading = getSteerAngle();
  }
  else
  {
     result = getSteerAngle();   // fills SteerAngle_degX10 if OK
     if (result != NaN)
     {
       Serial3.print("SENSOR {Steer ");
       Serial3.print(SteerAngle_degX10);
       Serial3.println("}");
     }
  }
...
{
   int data;
   data = DAC_Read(AngleCS);
   /* 
   data: leading 10 bits are angle (10 bits per 360 degrees) followed in order by
   OCF: Offset compensation finished; should be HIGH
   CDF: Cordic Overflow; should be LOW
   LIN: Linearity alarm; should be LOW
   MagInc: Magnitude Increase; HIGH if magnet is pushed toward IC
   MagDec: Magnitude Decrease: HIGH if magnet is pulled away from IC.  Both HIGH is bad.
   Even parity: ignore for now.
   */
   int Status = data & 0x38; // OCF, CDF, LIN
   if (Status != 0x20) return NaN;  // invalid data
   data >>= 6;
   SteerAngle_degX10 = ((data - StraightReading) *10 * 0x3FF) / 360;
   if (SteerAngle_degX10 > 1800)
     SteerAngle_degX10 -= 3600;
   if (SteerAngle_degX10 < -1800)
     SteerAngle_degX10 += 3600;   
   return (data);  // 10 bit result;
}
/*---------------------------------------------------------------------------------------*/ 
...
/*---------------------------------------------------------------------------------------*/ 
/* DAC_Read returns a 16 bit value from the address.
   Address is the chip select for the device, currently the AS5043 angle encoder.
*/
int DAC_Read (int address)
{
  int byte1, byte2;
  if (address != AngleCS) return NaN;
